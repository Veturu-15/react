...
function MobileList() {
    return (
        <div className='tv' style={{ display: 'flex', flexWrap: 'wrap', gap: '3%', padding: '3px' }}>
            {TvData.map((item) => (
                <div key={item.id} className='new' style={{  margin: '10px', color: 'black', padding: '10px' }}>
                    <Link to={`/tv/${item.id}`} style={{ textDecoration: 'none', color: 'black' }}>
                        <img src={item.images} alt={item.Brand} style={{ height: '350px', width: '300px', border:'1px solid black' }} />
